...
# Create Main Folder
mainDir = getwd()
subDir = "No_Outlier_removed_Sig_plots"
...
for (cur_dir in dir_names){  
  ...
  # Create One_Way Normality Plot Folder 
  mainDir = getwd()
  subDir = "Normality_Assumptions_One_Way"
  create_dir(mainDir,subDir)
  norm_one_wd = getwd()
  
  # Create One_Way Normality txt file
  create_txt_file(getwd(),"one_way_norm.txt")
  
  setwd("../") # Back to section folder
  
  # Create Two_Way Normality Plot Folder 
  mainDir = getwd()
  subDir = "Normality_Assumptions_Two_Way"
  create_dir(mainDir,subDir)
  norm_two_wd = getwd()
  
  # Create Two_Way Normality txt file
  create_txt_file(getwd(),"two_way_norm.txt")
  ...
  for (col_name in cur_col_names) {
    ...
    # Normality Check
    setwd(norm_one_wd)
    jpeg(paste(c(j,"_",col_name,"_norm.jpg"),collapse = ""), width = 1366, height = 700)
    plot(ggqqplot(data, col_name, facet.by = "emo_num"))
    dev.off()
    
    norm_out=data %>%
      group_by(emo_num) %>%
      shapiro_test(col_name)
    
    one_way_out=capture.output(col_name)
    cat("\nVariable Name", one_way_out, file="one_way_norm.txt", sep="\n", append=TRUE)
    one_way_out2=capture.output(norm_out) # main effect
    cat(one_way_out2, file="one_way_norm.txt", sep="\n", append=TRUE)
    ...
    setwd(norm_two_wd)
    jpeg(paste(c(j,"_",col_name,"_norm.jpg"),collapse = ""), width = 1366, height = 700)
    plot(ggqqplot(data, col_name, ggtheme = theme_bw()) + facet_grid(emo_num ~ Gender, labeller = "label_both"))
    dev.off()
    
    tryCatch({norm_out=data %>%
      group_by(Gender, emo_num) %>%
      shapiro_test(col_name)
    }, error=function(e){orm_out=0})
    
    two_way_out=capture.output(col_name)
    cat("\nVariable Name", two_way_out, file="two_way_norm.txt", sep="\n", append=TRUE)
    two_way_out2=capture.output(norm_out) # main effect
    cat(two_way_out2, file="two_way_norm.txt", sep="\n", append=TRUE)
